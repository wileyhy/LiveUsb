...
# shellcheck disable=SC2034
{
  ...
  alias als_function_boundary_in='local - _="${fn_bndry_lo} ${fn_bndry_sh} ${FUNCNAME[0]}() BEGINS ${fn_bndry_sh} ${fn_lvl} to $(( ++fn_lvl ))" loc_hyphn="$-" loc_exit_code="${EE:-$?}" loc_lineno="${LN:-$nL}"'
...
function setup_dnf(){ als_function_boundary_in
  ...
  # shellcheck disable=SC2206
  {
    ...
    # addl_pkgs+=( ${for_bashdb:=}       bash-devel make autoconf )
      ...
      addl_pkgs+=( ${for_critical_F:=}   xfce4-terminal )
      ...
      addl_pkgs+=( ${for_critical_K:=}   iptables-{libs,nft} {,python3-}nftables )
      addl_pkgs+=( ${for_critical_L:=}   dnf{,-data,-plugins-core} python3-dnf{,-plugins-core} {python3-,}libdnf )
    # addl_pkgs+=( ${for_careful_A:=}    systemd auditd sssd )
